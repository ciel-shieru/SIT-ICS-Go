...
func TestMergeEvents(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, mergedCount, _ := MergeEvents(tt.psEvents, tt.bsEvents)
...
func TestMergeEvents_LocationFilter(t *testing.T) {
	...
	result, mergedCount, _ := MergeEvents([]calendar.Event{campusPS}, []calendar.Event{zoomBS})
...
func TestMergeEvents_OnePSMatchedOnce(t *testing.T) {
	...
	result, mergedCount, _ := MergeEvents([]calendar.Event{psEvent}, []calendar.Event{bsEvent1, bsEvent2})
...
func TestDedupQuizzes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantReplacements: 1,
			wantVerify: func(t *testing.T, result []calendar.Event) {
				if result[0].QuizID != 99001 {
					t.Errorf("QuizID = %d, want 99001", result[0].QuizID)
				}
		...
		},
		{
			...
			quizEvents: []calendar.Event{
				...
				{Summary: "Quiz B", QuizID: 99013, Source: "brightspace-quizzes", DTStart: base.Add(2 * time.Hour), DTEnd: base.Add(3 * time.Hour)},
			...
			wantVerify: func(t *testing.T, result []calendar.Event) {
				...
				if !quizIDs[99010] {
					t.Error("expected matched quiz event with QuizID 99010")
				}
				if !quizIDs[99013] {
					t.Error("expected matched quiz event with QuizID 99013")
				}
				if !quizIDs[99015] {
		...
		},
		{
			...
			wantReplacements: 1,
			wantVerify: func(t *testing.T, result []calendar.Event) {
				if result[0].QuizID != 99005 {
					t.Errorf("QuizID = %d, want 99005", result[0].QuizID)
				}
				if result[0].CalendarEventID != 0 {
					t.Errorf("CalendarEventID = %d, want 0", result[0].CalendarEventID)
				}
		...
		{
			name: "duplicate calendar events by CalendarEventID",
			calendarEvents: []calendar.Event{
				{Summary: "Quiz Event A", CalendarEventID: 99020, QuizID: 0, Source: "brightspace-calendar", DTStart: base, DTEnd: base},
				{Summary: "Quiz Event A (dup)", CalendarEventID: 99020, QuizID: 0, Source: "brightspace-calendar", DTStart: base, DTEnd: base},
			},
			quizEvents: []calendar.Event{},
			wantTotalCount:   1,
			wantReplacements: 0,
			wantVerify: func(t *testing.T, result []calendar.Event) {
				...
				if result[0].CalendarEventID != 99020 {
					t.Errorf("CalendarEventID = %d, want 99020", result[0].CalendarEventID)
				}
			},
		},
		{
			...
			quizEvents: []calendar.Event{
				{Summary: "Quiz A", QuizID: 99030, Source: "brightspace-quizzes", DTStart: base.Add(-1 * time.Hour), DTEnd: base},
				{Summary: "Quiz B", QuizID: 99033, Source: "brightspace-quizzes", DTStart: base.Add(2 * time.Hour), DTEnd: base.Add(3 * time.Hour)},
			},
			wantTotalCount:   4,
			wantReplacements: 2,
			wantVerify: func(t *testing.T, result []calendar.Event) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, replacements, _ := DedupQuizzes(tt.calendarEvents, tt.quizEvents)
			if len(result) != tt.wantTotalCount {
...
func TestMergeEvents_ReturnsMatchedBSUIDs(t *testing.T) {
	loc, _ := time.LoadLocation("Asia/Singapore")
	base := time.Date(2026, 8, 31, 9, 0, 0, 0, loc)
...
func TestMergeEvents_MultipleMatches(t *testing.T) {
	loc, _ := time.LoadLocation("Asia/Singapore")
	base := time.Date(2026, 8, 31, 9, 0, 0, 0, loc)

	psEvent1 := calendar.Event{
		CourseCode: "MOD1001",
		Location:   "Online",
		DTStart:    base,
		DTEnd:      base.Add(2 * time.Hour),
	}
	psEvent2 := calendar.Event{
		CourseCode: "COR2003",
		Location:   "Online",
...
func TestDedupQuizzes_ReturnsRemovedCalendarEventIDs(t *testing.T) {
	...
	calendarEvent1 := calendar.Event{
		Summary:         "Quiz Event A",
		CalendarEventID: 99010,
		QuizID:          99010,
...
func TestDedupQuizzes_NoQuizMatches(t *testing.T) {
	loc, _ := time.LoadLocation("Asia/Singapore")
	base := time.Date(2026, 10, 18, 15, 59, 59, 0, loc)

	calendarEvent := calendar.Event{
		Summary:         "Lecture",
		CalendarEventID: 99020,
		QuizID:          0,
		Source:          "brightspace-calendar",
		DTStart:         base,
		DTEnd:           base,
	...
	if replacements != 0 {
		t.Errorf("replacements = %d, want 0", replacements)
	}
	if len(result) != 2 {
